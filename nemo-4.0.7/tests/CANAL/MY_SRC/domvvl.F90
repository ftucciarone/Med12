   ...
   SUBROUTINE dom_vvl_rst( kt, cdrw )
      ...
      IF( TRIM(cdrw) == 'READ' ) THEN        ! Read/initialise 
         ...
         ELSE                                   !* Initialize at "rest"
            ...
            ELSE
              !
               ! usr_def_istate called here only to get sshb, that is needed to
               ! initialize e3t_b and e3t_n 
               CALL usr_def_istate( gdept_0, tmask, tsb, ub, vb, sshb  )
               ! usr_def_istate will be called again in istate_init to
               ! initialize ts(bn), ssh(bn), u(bn) and v(bn)
               !
               DO jk=1,jpk
