...
! components on the nemo V grid, and extract the v component on the boundary
! ----------------------------------------------------------------------
       program create_bdy_V
...
! ------------------------------------------------------
    ...
    count2(3)=jptdta
       ...
       if(ll_rotate) then
    stat = nf90_inq_varid (ncid, 'glamv', idvar)
    if (stat /= nf90_noerr) call netcdf_err(stat)
    stat = nf90_get_var (ncid,idvar,lon_in(:,:),start2,count2)
    if (stat /= nf90_noerr) call netcdf_err(stat)

    stat = nf90_inq_varid (ncid, 'gphiv', idvar)
    if (stat /= nf90_noerr) call netcdf_err(stat)
...
! ------------------------------------------------------
      ...
      if (stat /= nf90_noerr) call netcdf_err(stat)
!
      stat = nf90_inq_dimid (ncid, 'xbV', idvar)
      if (stat /= nf90_noerr) call netcdf_err(stat)
...
!
      ...
      stat = nf90_inq_varid (ncid, 'lonV', idvar)
      if (stat /= nf90_noerr) call netcdf_err(stat)
      stat = nf90_get_var (ncid,idvar,lon_ou)
      if (stat /= nf90_noerr) call netcdf_err(stat)
!
      
      stat = nf90_inq_varid (ncid, 'latV', idvar)
      if (stat /= nf90_noerr) call netcdf_err(stat)
      stat = nf90_get_var (ncid,idvar,lat_ou)
      if (stat /= nf90_noerr) call netcdf_err(stat)

      stat = nf90_inq_varid (ncid, 'nbiv', idvar)
      if (stat /= nf90_noerr) call netcdf_err(stat)
      stat = nf90_get_var (ncid,idvar,indi)
      if (stat /= nf90_noerr) call netcdf_err(stat)
!
      stat = nf90_inq_varid (ncid, 'nbjv', idvar)
      if (stat /= nf90_noerr) call netcdf_err(stat)
...
! ------------------------------------------------------
       do jpo=1,jpiout
           u_ou(jpo,1,:,:)=vmodel(indi(jpo),indj(jpo),:,:)
       enddo
!-----------------------------------------------------------------------
! Write netcdf file
!-----------------------------------------------------------------------
...
!
  stat = nf90_def_var ( ncid, 'vomecrty', nf90_float, (/ idlon, idlat, iddep, ideof/), idvaru)
!
